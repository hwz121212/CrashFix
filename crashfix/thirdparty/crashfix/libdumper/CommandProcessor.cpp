...
int CCommandProcessor::ReadDump(LPCSTR szFileName, LPCSTR szOutFile)
{
	...
	for(i=0; i<dmp.GetThreadCount(); i++)
	{
		...
		fprintf(f, "%d. Thread ID = %u Teb=%llu StackStart=%llu StackSize=%u CtxRva=%u\n",
		...
		for(i=0; i<pMiniDump->GetThreadCount(); i++)
		{
			...
			if(bInit)
			{
				...
				while(bGetFrame)
				{
					...
					{
						sprintf(szBuffer, "0x%llx", pStackFrame->m_dwAddrPC);
